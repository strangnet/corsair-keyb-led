...
#	pragma message("WARNING: CUESDK deprecation warnings were disabled")
	...
	{
		enum CorsairChannelDeviceType type;	// type of the LED-device.
	...
	struct CorsairChannelInfo				// contains information about separate channel of the DIY-device.
	{
		...
		struct CorsairChannelDeviceInfo* devices;	// array containing information about each separate LED-device connected to the channel controlled by the DIY device. Index of the LED-device in array is same as the index of the LED-device connected to the DIY-device.
	};
	...
	struct CorsairChannelsInfo			// contains information about channels of the DIY-devices.
	{
		...
		struct CorsairChannelInfo* channels;	// array containing information about each separate channel of the DIY-device. Index of the channel in the array is same as index of the channel on the DIY-device.
	};

	struct CorsairDeviceInfo	// contains information about device.
	{
		enum CorsairDeviceType type;               // enum describing device type.
		const char* model;                    // null - terminated device model(like “K95RGB”).
		enum CorsairPhysicalLayout physicalLayout; // enum describing physical layout of the keyboard or mouse.
		enum CorsairLogicalLayout logicalLayout;   // enum describing logical layout of the keyboard as set in CUE settings.
		...
		struct CorsairChannelsInfo channels;         // structure that describes channels of the DIY-devices.
	...
	{
		enum CorsairLedId ledId;				// identifier of led.
		double top;
		double left;
		double height;
	...
	struct CorsairLedPositions	// contains number of leds and arrays with their positions.
	{
		...
		struct CorsairLedPosition* pLedPosition; // array of led positions.
	};

	struct CorsairLedColor		// contains information about led and its color.
	{
		enum CorsairLedId ledId;				// identifier of LED to set.
	...
	{
		CorsairDeviceId deviceId; // null-terminated string that contains unique device identifier.
		enum CorsairKeyId keyId;       // G or M key that was pressed/released.
		bool isPressed;           // true if pressed, false if released.
	};

	struct CorsairEvent // contains information about event id and event data.
	{
		enum CorsairEventId id; // event identifier.
		union
		{
			const struct CorsairDeviceConnectionStatusChangedEvent *deviceConnectionStatusChangedEvent; // when id == CEI_DeviceConnectionStatusChangedEvent contains valid pointer to structure with information about connected or disconnected device.
			const struct CorsairKeyEvent *keyEvent;                                                     // when id == CEI_KeyEvent contains valid pointer to structure with information about pressed or released G or M button and device where this event happened.
		};
	};

	typedef void(*CorsairEventHandler)(void *context, const struct CorsairEvent *event); // defines a callback that will be called by SDK to notify user about events.
	...
	CORSAIR_LIGHTING_SDK_EXPORT bool CorsairSetLedsColors(int size, struct CorsairLedColor* ledsColors);
	...
	CORSAIR_LIGHTING_SDK_EXPORT bool CorsairSetLedsColorsBufferByDeviceIndex(int deviceIndex, int size, struct CorsairLedColor* ledsColors);
	...
	CORSAIR_LIGHTING_SDK_EXPORT bool CorsairSetLedsColorsFlushBufferAsync(void (*callback)(void *context, bool result, enum CorsairError error), void *context);

	// get current color for the list of requested LEDs.
	CORSAIR_LIGHTING_SDK_EXPORT bool CorsairGetLedsColors(int size, struct CorsairLedColor* ledsColors);

	// get current color for the list of requested LEDs for specified device.
	CORSAIR_LIGHTING_SDK_EXPORT bool CorsairGetLedsColorsByDeviceIndex(int deviceIndex, int size, struct CorsairLedColor* ledsColors);
	...
	CORSAIR_LIGHTING_SDK_EXPORT bool CorsairSetLedsColorsAsync(int size, struct CorsairLedColor* ledsColors, void(*CallbackType)(void*, bool, CorsairError), void *context);
	...
	CORSAIR_LIGHTING_SDK_EXPORT struct CorsairDeviceInfo *CorsairGetDeviceInfo(int deviceIndex);

	// provides list of keyboard LEDs with their physical positions.
	CORSAIR_LIGHTING_SDK_EXPORT struct CorsairLedPositions *CorsairGetLedPositions();
	...
	CORSAIR_LIGHTING_SDK_EXPORT struct CorsairLedPositions *CorsairGetLedPositionsByDeviceIndex(int deviceIndex);

	// retrieves led id for key name taking logical layout into account.
	CORSAIR_LIGHTING_SDK_EXPORT enum CorsairLedId CorsairGetLedIdForKeyName(char keyName);
	...
	CORSAIR_LIGHTING_SDK_EXPORT bool CorsairRequestControl(enum CorsairAccessMode accessMode);

	// checks file and protocol version of CUE to understand which of SDK functions can be used with this version of CUE.
	CORSAIR_LIGHTING_SDK_EXPORT struct CorsairProtocolDetails CorsairPerformProtocolHandshake();

	// returns last error that occured while using any of Corsair* functions.
	CORSAIR_LIGHTING_SDK_EXPORT enum CorsairError CorsairGetLastError();

	// releases previously requested control for specified access mode.
	CORSAIR_LIGHTING_SDK_EXPORT bool CorsairReleaseControl(enum CorsairAccessMode accessMode);
	...
	CORSAIR_LIGHTING_SDK_EXPORT bool CorsairRegisterKeypressCallback(void (*CallbackType)(void *context, enum CorsairKeyId keyId, bool pressed), void *context);

	// reads boolean property value for device at provided index.
	CORSAIR_LIGHTING_SDK_EXPORT bool CorsairGetBoolPropertyValue(int deviceIndex, enum CorsairDevicePropertyId propertyId, bool *propertyValue);

	// reads integer property value for device at provided index.
	CORSAIR_LIGHTING_SDK_EXPORT bool CorsairGetInt32PropertyValue(int deviceIndex, enum CorsairDevicePropertyId propertyId, int *propertyValue);
